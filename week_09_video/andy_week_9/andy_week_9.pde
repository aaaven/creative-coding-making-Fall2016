...
void setup() {
  ...
  myMovie = new Movie(this, "av homework10.mp4");
  myMovie.loop();  
  columnCount = 120; // how many columns to go
  columnIndex = 0;
...
void draw() {
  if (myMovie.available() && columnIndex < columnCount) {
    ...
    float pos = map(columnIndex, 0, columnCount, 0.001*myMovie.duration(),myMovie.duration()); // map the whole video to columnCount
    myMovie.jump(pos);
    myMovie.read();
